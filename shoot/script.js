document.addEventListener("DOMContentLoaded", function(event) {
 ...
 function updateScore() {
   ...
   document.getElementById("accuracy").innerHTML = "Time left: " + timerValue + "s";
 }
    ...
    function updateTimer() {
      ...
      accuracyElement.textContent = "Time left: " + timerValue + "s";
      if (timerValue === 0) {
        ...
        // Stop the game logic (remove event listeners, etc.)
        stopGame();
      }
      timerValue--;
    }
    ...
    function startTimer() {
      if (!timerInterval) { // Only start if the timer isn't already running
        timerValue = 30; // Reset timer value if needed
    ...
    function resetGame() {
      // Reset your game-related variables and elements here
      shots = 0;
      score = 0;

      // Reset the shots and score in the HTML
      updateScore();
      ...
      function stopGame() {
      // Remove event listeners and other game-related logic
      ball.removeEventListener("mousedown", grabBall);
      ball.removeEventListener("touchstart", grabBall);
      document.removeEventListener("mousemove", moveBall);
      document.removeEventListener("touchmove", moveBall);
      // Other logic to reset game state
    }
      ...
      const playAgainButtonPopup = document.getElementById("play-again-popup");
    playAgainButtonPopup.addEventListener("click", closeGameOverPopup);

    // Event listener for the "Play Again" button
    const playAgainButton = document.getElementById("play-again");
    playAgainButton.addEventListener("click", resetGame);

    // Call the startTimer function to begin the timer
    startTimer();
     };
    });